  ...
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        const safeEnqueue = (eventName: string, payload: unknown) => {
          if (streamClosed) return;
          try {
            controller.enqueue(encoder.encode(sseEvent(eventName, payload)));
          } catch (err) {
            streamClosed = true;
            console.error('[CHAT] safeEnqueue failed:', err);
          }
        };
        ...
        const safeClose = () => {
          ...
          try {
            ...
            console.error('[CHAT] safeClose failed:', err);
          }
        };
        ...
        try {
          ...
          while (iterations < maxIterations) {
            ...
            for await (const chunk of streamCompletion) {
              ...
              if (delta.content) {
                ...
                safeEnqueue('delta', { content: delta.content });
              }
            }
            ...
            if (hasToolCalls) {
              ...
              for (const tc of toolCallsArray) {
                ...
                if (statusText) {
                  safeEnqueue('status', { text: statusText });
                }
                  ...
                  switch (toolName) {
                    ...
                    case 'create_listing': {
                      ...
                      if (!sellerCheck || !sellerCheck.isProfileComplete) {
                        safeEnqueue('seller_action_required', {
                      ...
                      safeEnqueue('listing_draft', draftData);
          ...
          if (iterations >= maxIterations && !assistantResponse) {
            ...
            safeEnqueue('delta', { content: assistantResponse });
          }
          ...
          if (!streamClosed) {
            safeEnqueue('delta', { content: errorMsg });
            safeEnqueue('done', { session_id: sessionId, error: true });
            safeClose();
          }
        }
      },
